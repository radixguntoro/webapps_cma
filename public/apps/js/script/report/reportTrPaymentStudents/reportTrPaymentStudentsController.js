var app = angular.module('reportTrPaymentStudentsCtrl', ['factoryReportTrPaymentStudents']);

app.controller('reportTrPaymentStudentsController', function($rootScope, $scope, $stateParams, $window, $timeout, $location, $http, $state, $filter, toastr, hotkeys, reportTrPaymentStudentsFactory, studentFactory) {
    //##############################//
    //      DEKLARASI VARIABLE      //
    //##############################//
    ...
    $scope.filterData = function() {
        if ($scope.report.datestart == null || $scope.report.dateend == null) {
            ...
            reportTrPaymentStudentsFactory.filterDataReportTrPaymentStudents(date_start, date_end, student_id)
            .then(function() {
                $scope.data = reportTrPaymentStudentsFactory.resultData;
                $scope.totalItems = reportTrPaymentStudentsFactory.totalItems;
                $scope.totalTrPayments = reportTrPaymentStudentsFactory.totalTrPayments;
                $scope.totalReturnSales = reportTrPaymentStudentsFactory.totalReturnSales;
    ...
    $scope.exportExcel = function() {
        ...
        if ($scope.report.datestart == null || $scope.report.dateend == null) {
            ...
            var url = 'reportTrPaymentStudents/exportExcel?student=' + student_id + '&datestart=' + date_start + '&dateend=' + date_end;
            $window.open(url);
            $scope.loading = false;
        }
    }
    ...
    $scope.showTrPaymentsDetail = function(tr_payments) {
        ...
        reportTrPaymentStudentsFactory.showTrPaymentsDetail({
            tr_payments_id: tr_payments.id
        }).then(function() {
            $scope.tr_payment_item_detail = reportTrPaymentStudentsFactory.dataTrPaymentsDetail.tr_payment_item_detail;
            $scope.tr_payment_tuition_detail = reportTrPaymentStudentsFactory.dataTrPaymentsDetail.tr_payment_tuition_detail;
            $scope.tr_payment_deposit_detail = reportTrPaymentStudentsFactory.dataTrPaymentsDetail.tr_payment_deposit_detail;
