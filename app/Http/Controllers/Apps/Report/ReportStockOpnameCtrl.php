...
{
    public function filterReportStockOpname(Request $request)
    {
    ...
    public function exportExcel(Request $request)
    {
        ...
        if ($datestart || $dateend) {
            ...
            $export_cxcel = Excel::create('REP-STOCKOPNAMEINVENTORY-'.$periode_start.'-'.$periode_end, function($excel) use ($data_excel) {
                $excel->sheet('SHEET', function($sheet) use ($data_excel) {
                    $sheet->loadView('templates.apps.export.excelStockOpname', compact('data_excel'));
                });
            })->export('xls')->only('id');
